-- ====================================================================
-- Orbit Meeting — Consolidated Database Schema
-- ====================================================================
...
-- ================================================================
-- 1. CLEANUP (safe to re-run — drops everything first)
--    CASCADE drops on tables remove all policies, triggers,
--    indexes, and FK constraints automatically.
-- ================================================================

-- Drop trigger on auth.users (always exists in Supabase)
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;

-- Drop tables in dependency order (children before parents).
-- CASCADE handles policies, triggers, indexes, and FK constraints.
...
-- Drop functions (now safe — dependent triggers dropped by table CASCADE above)
...
-- 6. CHAT MESSAGES
-- ================================================================
-- meeting_id is TEXT because rooms use arbitrary UUID strings that
-- don't always have a corresponding meetings row.
-- user_id is TEXT to accept both auth UUIDs and anonymous peer-xxx
-- identities.
...
-- --- CHAT MESSAGES ---
-- Authenticated participants can SELECT (via meeting_participants or as creator)
-- Anonymous users can SELECT too (meeting_id filters naturally)
-- Anyone can INSERT (authenticated or anonymous)
...
CREATE INDEX IF NOT EXISTS idx_mp_meeting ON public.meeting_participants(meeting_id);
CREATE INDEX IF NOT EXISTS idx_mp_user ON public.meeting_participants(user_id);
...
CREATE INDEX IF NOT EXISTS idx_chat_meeting ON public.chat_messages(meeting_id);

-- ================================================================
-- 9. PROFILE UPDATE TRIGGER (must be after profiles table is created)
-- ================================================================
...
-- ================================================================
-- VERIFICATION QUERIES (run to confirm everything is set up)
-- ================================================================
